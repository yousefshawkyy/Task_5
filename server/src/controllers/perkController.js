...
// get all perks (only for logged-in user)
...
// 🔍 OPTIONAL FILTER ENDPOINT (this is what your routes import as `filterPerks`)
export async function filterPerks(req, res, next) {
  try {
    const { search, merchant, category } = req.query;

    const query = {};

    // If you want to filter for the current user's perks only:
    if (req.user?.id) {
...
export async function deletePerk(req, res, next) {
  try {
    ...
    // keep response simple; frontend just needs to know it's gone
